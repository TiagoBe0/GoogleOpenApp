...
export default async function TurnosPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Upcoming */}
      <div>
        <h2 className="font-semibold text-gray-900 mb-3">Próximos turnos</h2>
        {upcoming.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center text-gray-400">
            <svg className="w-10 h-10 mx-auto mb-3 text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            <p className="text-sm">No tenés turnos próximos</p>
            {profile?.slug && (
              <p className="text-xs mt-1">
                Compartí tu <a href={`/p/${profile.slug}`} target="_blank" rel="noopener noreferrer" className="text-indigo-500 hover:underline">perfil público</a> para recibir reservas.
              </p>
            )}
          </div>
        ) : (
